...
KNOB_DESC = {
    'verify_depth': '结果校验深度 0=不校验 1=只查最终步 2=每步都查 (对静默损坏族强; 对显性报错/盲步/强噪声族基本无效=纯开销)',
    'retries': '失败重试次数 0-8 (对显性报错族强; 对系统性逻辑错误/盲步/噪声无效; 消耗预算)',
    'task_decompose': '任务分解 0=整体算(系统性逻辑错误,重试无效) 1/2=拆步(错误变随机,可被重试挽救)',
    'multi_path_voting': '多路径交叉校验 0=关 3=三路 5=五路 (独立路径同时错在同一处才失败; 需 diversity_seed>=1 才真正独立)',
    'memory_read': '读历史同构任务经验 0/1 (多步任务族中降低错误率)',
    'memory_write': '写经验到记忆库 0/1 (与read配合才有收益; 单独开是纯开销)',
    'timeout_budget': 'probe/时间预算 1-8 (部分可观测族必须足够预算探明盲步, 否则盲步错误无法校验)',
    'early_exit': '高置信提前退出 0/1 (省下的计算转为+1个probe)',
    'tool_choice': '工具选择 fast_risky=快但错率高 slow_safe=慢但稳 both=双路',
    'diversity_seed': '路径多样性种子 0=路径相关(共享错误,投票无效) 1-3=独立 (让多路交叉校验真正生效)',
}
...
FAMILY_DESC = {
    ...
    'C': '多步依赖族: 4步串行计算, 每步25%逻辑错误。不分解(task_decompose=0)时错误是系统性的, 重试救不回, 成功率天花板约0.32',
    'D': '部分可观测族: 4步中只有前2步信息可见, 后2步必须用probe预算(timeout_budget)探明。盲步错误无法校验(vd/retries无效), 天花板约0.30',
    'E': '噪声标签族: 执行错误率30%且校验信号不可靠(verify/retries基本无效)。独立多路径交叉校验(multi_path_voting>=3 + diversity_seed>=1)是唯一可靠手段',
}
...
def solve_C(task, cfg, rng, mem):
    ...
    if cfg['memory_read'] and cfg['memory_write']:
        ...
        p -= 0.03
    vd = cfg['verify_depth']
    # v3: 未分解时逻辑错误是系统性的——同一步重试再多次还是同一个错
    systematic = cfg['task_decompose'] == 0

    def one_path():
        for s in range(steps):
            bad = rng.random() < p
            if bad and systematic:
                if vd >= 1:
                    return (False, 'systematic_logic_error_detected_step%d' % s)
                return (False, 'systematic_logic_error_accepted_step%d' % s)
            if bad and vd >= 1:
                for a in range(cfg['retries']):
                    if rng.random() >= p:
                        ...
                        break
                if bad:
                    return (False, 'logic_error_retry_exhausted_step%d' % s)
            elif bad:
                return (False, 'silent_logic_error_step%d' % s)
        return (True, '')
            ...
            if s < probed:
                ...
                if vd >= 1:
                    for a in range(cfg['retries'] + 1):
                        ...
                        return (False, 'probed_step_error_step%d' % s)
                else:
                    if rng.random() < step_p:
                        return (False, 'probed_step_error_step%d' % s)
            else:
                # v3: 盲步=未探明信息, 校验无从谈起(vd/retries 无效), 错了也不知道
                if rng.random() < 0.45:
                    return (False, 'blind_step_error_step%d' % s)
        return (True, '')
...
def solve_E(task, cfg, rng, mem):
    ...
    p = 0.30
    vd = cfg['verify_depth']
    # v3: 校验信号不可靠: vd 只带来微弱收益, retries 完全无效(重试结果一样不可信)
    if vd >= 1:
        p *= 0.85
    k = cfg['multi_path_voting']
    # 交叉校验: 多条独立(diversity>=1)路径只有同时错在同一处才失败
    if k >= 3 and cfg['diversity_seed'] >= 1:
        p = p ** k

    def one_path():
        for s in range(steps):
            if rng.random() < p:
                return (False, 'noise_exec_error_step%d' % s)
        return (True, '')

    if k > 0 and cfg['diversity_seed'] >= 1:
        return one_path()  # 交叉校验语义已在 p 中体现
    if k > 0:
        if cfg['diversity_seed'] == 0:
            k2 = 1 + (k - 1) // 2
        return _vote([one_path() for _ in range(k2)])
    return one_path()
...
def main():
    ...
    with open('/root/private_data/phase_d1_v3_knobs.json', 'w') as f:
        json.dump(RESULT, f, ensure_ascii=False, indent=1)
    log('saved /root/private_data/phase_d1_v3_knobs.json')
    log('D1_DONE')
