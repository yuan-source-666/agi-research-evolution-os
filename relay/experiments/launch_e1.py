#!/usr/bin/env python3
"""上传 phase_e1_lora.py 到 SCNet，等 D2 (PID 5290) 跑完后自动启动 E1。"""
...
LOCAL = r"E:\ai_agent_workspace\teleagent\.temp\scnet_v8\phase_e1_lora.py"
REMOTE = "/root/private_data/phase_e1_lora.py"
D2_PID = "5290"
...
print("== launch (waits for D2 pid %s) ==" % D2_PID)
# 用 PID 等待（避免 pgrep 自匹配），D2 结束后 30 秒缓冲再跑 E1
code = (
    "import subprocess, os\n"
    "d = '/root/private_data'\n"
    "wrapper = ('while kill -0 " + D2_PID + " 2>/dev/null; do sleep 30; done; "
    "sleep 30; cd /root/private_data && python3 phase_e1_lora.py "
    "> phase_e1_lora.log 2>&1')\n"
    "log = open(os.path.join(d, 'phase_e1_wrapper.log'), 'w')\n"
    "p = subprocess.Popen(['bash', '-c', wrapper], stdout=log,\n"
    "                     stderr=subprocess.STDOUT, start_new_session=True)\n"
    "print('wrapper pid', p.pid)")
print(run_code(code))
print("log: /root/private_data/phase_e1_lora.log")
print("result: /root/private_data/phase_e1_lora.json")
