# -*- coding: utf-8 -*-
"""阶段 C v3：B 族 best-of-k 多样性采样提案器。

v2 病根：B 族首提案被门禁拒绝后，1.5B 在 4 次重试里全部输出同一 JSON，
被去重判死，5 轮全 skip。
v3 修复：每轮独立采样 k=6 个候选（temp=0.8），
过滤 no-op / 历史重复后取第一个合法提案——不靠反馈重试靠覆盖面。
闭环 HISTORY 反馈保留。只跑 B 族。
"""
import copy
import json
import random
...
N_ROUNDS = 5
K_SAMPLES = 6
OUT_JSON = '/root/private_data/phase_c3_llm_proposer.json'

RESULT = {'phase': 'C3_llm_proposer_bestofk',
...
def evolve_family_llm(family, tasks, proposer):
    ...
    memory = MemorySystem(store_path='/root/private_data/evo_memory_C3_%s.sqlite' % family)
    loop = EvolutionLoop(memory=memory,
                         log_dir='/root/private_data/evo_logs_C3_%s' % family)
    ...
    for rnd in range(1, N_ROUNDS + 1):
        ...
        if prop is None:
            print('[%s/R%d] no valid candidate in %d samples, round skipped' %
                  (family, rnd, K_SAMPLES), flush=True)
...
def main():
    ...
    print('PHASE_C3_DONE', flush=True)
