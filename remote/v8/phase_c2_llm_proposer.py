# -*- coding: utf-8 -*-
"""阶段 C v2：闭环 LLM 提案器（拒绝反馈 + 去重）。

v1 缺口：
  1) LLM 看不到自己上一轮被门禁拒绝及原因 -> 反复提同一方案；
  2) 已生效配置重复提案（no-op）。
v2 修复：
  1) prompt 注入 HISTORY：每轮提案、是否接受、门禁实测 sr 与拒绝原因，
     并要求"不要重复被拒/已生效的提案，提出更强或不同的改动"；
  2) 解析层去重：与当前配置相同的提案或与历史被拒提案相同的，
     视为无效，带反馈重试；
  3) 轮数 3 -> 5，给迭代留空间。
"""
import copy
...
EVAL_N = 600
N_ROUNDS = 5
OUT_JSON = '/root/private_data/phase_c2_llm_proposer.json'

RESULT = {'phase': 'C2_llm_proposer_closed_loop',
          'ts': time.strftime('%Y-%m-%d %H:%M:%S'),
          'model': MODEL_PATH, 'p_fail': P_FAIL, 'n_tasks': N_TASKS,
          'n_rounds': N_ROUNDS, 'families': {}}
...
# ============ 任务族（与 phase_b_transfer / phase_c v1 完全一致） ============
...
class Proposer:
    ...
    def chat(self, user, temperature=0.4, max_new_tokens=220):
...
def build_prompt(family, cfg, stats, fail_samples, ok_sample, history):
    fam_desc = ('Family A: the tool sometimes returns a WRONG value that looks '
                'valid (silent corruption).'
                if family == 'A' else
    ...
    lines = [
        ...
        '',
        'ENVIRONMENT',
        '- Tool "compute" answers arithmetic tasks. ' + fam_desc,
        '- Agent config knobs:',
    ...
    if history:
        lines += [
            ...
            '  STRONGER or DIFFERENT change (e.g. a different knob value).',
        ]
        for h in history:
...
def is_duplicate(changes, rejected_list):
    return any(c == changes for c in rejected_list)


def ask_proposal(proposer, family, cfg, stats, fail_samples, ok_sample, history):
    """带反馈重试的提案请求。重试原因包括解析失败、no-op、重复被拒提案。"""
    ...
    feedback = ''
    for att in range(4):
        user = prompt if not feedback else (
            prompt + '\n\nPREVIOUS OUTPUT REJECTED: ' + feedback +
            '\nOutput ONLY a corrected, DIFFERENT JSON proposal.')
        t0 = time.time()
        raw = proposer.chat(user)
        lat = round(time.time() - t0, 2)
        prop, err = parse_proposal(raw)
        if prop is not None:
            if is_noop(prop['changes'], cfg):
                err = 'proposal equals current config (no-op); change at least one knob'
                prop = None
            elif is_duplicate(prop['changes'], rejected_before):
                err = 'proposal was already rejected before; propose something different'
                prop = None
        rounds.append({'attempt': att + 1, 'latency_s': lat,
                       'raw': raw[-800:], 'error': err})
        print('[llm/%s/att%d] %.1fs -> %s' %
              (family, att + 1, lat, (prop['changes'] if prop else err)), flush=True)
        if prop is not None:
            return prop, rounds
        feedback = err or 'invalid'
    return None, rounds
...
def evolve_family_llm(family, tasks, proposer):
    ...
    memory = MemorySystem(store_path='/root/private_data/evo_memory_C2_%s.sqlite' % family)
    loop = EvolutionLoop(memory=memory,
                         log_dir='/root/private_data/evo_logs_C2_%s' % family)
    ...
    for rnd in range(1, N_ROUNDS + 1):
        ...
        prop, rounds = ask_proposal(proposer, family, copy.deepcopy(cfg),
                                    stats, fails, ok_sample, history)
        rec = {'round': rnd, 'base_sr': base_sr,
               'llm_rounds': rounds, 'proposal': prop, 'accepted': None,
               'bench_sr': None, 'gate': None, 'parse_ok': prop is not None}

        if prop is None:
            print('[%s/R%d] LLM failed to produce a valid new proposal, round skipped' %
...
def main():
    print('==== Phase C v2: closed-loop LLM auto-proposer ====', flush=True)
    proposer = Proposer()

    tasks = make_tasks(N_TASKS)

    resA = evolve_family_llm('A', tasks, proposer)
    print('A_llm2* =', resA['final_config'], flush=True)
    resB = evolve_family_llm('B', tasks, proposer)
    print('B_llm2* =', resB['final_config'], flush=True)

    evalA = make_tasks(EVAL_N, seed=6001)
    evalB = make_tasks(EVAL_N, seed=6002)
    configs = {
        'base': {'verify': False, 'retries': 0},
        'A_llm2_star': resA['final_config'],
        'B_llm2_star': resB['final_config'],
        'A_manual_star': {'verify': True, 'retries': 4},
    ...
    print('PHASE_C2_DONE', flush=True)
