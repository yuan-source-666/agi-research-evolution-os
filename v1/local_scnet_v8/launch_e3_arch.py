#!/usr/bin/env python3
"""E3-ARCH 启动器：杀掉 E2-STRUCT（在 Qwen 上砍层不算改架构，老板已否决），
上传并启动从零架构自进化 phase_e3_arch.py，E1 修复版排在其后自动接力。"""
...
# 1) 杀掉 E2-STRUCT 及其 E1 wrapper（架构方向已被否决，腾出 GPU）
kill_code = (
    "import subprocess\n"
    "print(subprocess.run(['bash','-lc',\n"
    "  'kill 8662 8661 2>/dev/null; pkill -f phase_e2_struct 2>/dev/null; "
    "sleep 3; ps aux | grep -E \"phase_e\" | grep -v grep | head -5; "
    "nvidia-smi --query-gpu=memory.used --format=csv,noheader'],\n"
    "  capture_output=True, text=True).stdout)")
print("== killing E2-STRUCT ==")
print(run_code(kill_code))

# 2) 上传 E3-ARCH
b64 = base64.b64encode(open(
    r"[LOCAL-WORKSPACE]\teleagent\.temp\scnet_v8\phase_e3_arch.py",
    "rb").read()).decode()
api_put("/root/private_data/phase_e3_arch.py", b64)
print("uploaded /root/private_data/phase_e3_arch.py")

# 3) 启动 E3，E1 修复版排在其后自动接力
code = (
    "import subprocess\n"
    ...
    "> phase_e1_lora.log 2>&1') % p3.pid\n"
    "logw = open('/root/private_data/phase_e1_wrapper3.log', 'w')\n"
    "pw = subprocess.Popen(['bash', '-c', wrapper], stdout=logw,\n"
    "                      stderr=subprocess.STDOUT, start_new_session=True)\n"
    "print('e1 rerun wrapper pid', pw.pid)")
print("== launching E3-ARCH ==")
print(run_code(code))
