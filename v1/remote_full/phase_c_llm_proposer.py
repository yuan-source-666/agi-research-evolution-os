# -*- coding: utf-8 -*-
"""阶段 C：LLM 自动生成改进提案（元-元层第三步）。

此前 A/B 族的 PROPOSALS 是手写剧本；本脚本让 Qwen2.5-1.5B-Instruct
读取 observe() 统计与失败 trace 样本，自行诊断故障模式并输出结构化
JSON 提案（changes: {verify, retries}），再走真实的
沙箱 -> 基准 -> 全量回归门禁 -> decide 链路。

对照：手写剧本结果 phase_b_transfer.json（A*={verify,retries=4},
B*={retries=4}）。评价维度：
  1) JSON 解析/模式合法率（提案格式质量）
  2) 门禁接受率（提案方向正确率）
  3) 最终 sr 与手写剧本的差距
  4) 提案延迟
"""
...
EVAL_N = 600
N_ROUNDS = 3
OUT_JSON = '/root/private_data/phase_c_llm_proposer.json'

RESULT = {'phase': 'C_llm_proposer', 'ts': time.strftime('%Y-%m-%d %H:%M:%S'),
...
class Proposer:
    def __init__(self):
        ...
        print('[proposer] loading model (bf16, cuda)...', flush=True)
        t0 = time.time()
        try:
    ...
    def chat(self, user, temperature=0.3, max_new_tokens=220):
...
def build_prompt(family, cfg, stats, fail_samples, ok_sample):
...
def parse_proposal(text):
    """从模型输出抽取并校验提案 JSON。返回 (proposal|None, err)。"""
...
def ask_proposal(proposer, family, cfg, stats, fail_samples, ok_sample):
    """带 3 次重试的提案请求。返回 (proposal|None, rounds)。"""
    prompt = build_prompt(family, cfg, stats, fail_samples, ok_sample)
    rounds = []
    feedback = ''
    for att in range(3):
...
def evolve_family_llm(family, tasks, proposer):
    ...
    memory = MemorySystem(store_path='/root/private_data/evo_memory_C_%s.sqlite' % family)
    loop = EvolutionLoop(memory=memory,
                         log_dir='/root/private_data/evo_logs_C_%s' % family)
    ...
    for rnd in range(1, N_ROUNDS + 1):
        ...
        if prop is None:
            print('[%s/R%d] LLM failed to produce valid JSON, round skipped' %
        ...
        print('[%s/R%d] %s accepted=%s bench=%s gate=%s' %
              (family, rnd, prop['changes'], ok, rec['bench_sr'],
               (gate_info or {}).get('detail', '-')), flush=True)
        fam_state['proposals'].append(rec)
...
def main():
    print('==== Phase C: LLM auto-proposer ====', flush=True)
    proposer = Proposer()

    tasks = make_tasks(N_TASKS)

    resA = evolve_family_llm('A', tasks, proposer)
    print('A_llm* =', resA['final_config'], flush=True)
    resB = evolve_family_llm('B', tasks, proposer)
    print('B_llm* =', resB['final_config'], flush=True)
    ...
    configs = {
        ...
        'A_llm_star': resA['final_config'],
        'B_llm_star': resB['final_config'],
